            ...
            steps {
                script {
                    ...
                    withCredentials([
                        ...
                        container('maven') {
                            try {
                                sh 'mvn clean test -DsuiteXmlFile="src/test/resources/Parallel.xml" -Dsaucelabs.url=${SAUCELABS_URL} -Dsaucelabs.username=${SAUCELABS_USR} -Dsaucelabs.accessKey=${SAUCELABS_PWD} || true'
                            } catch (err) {
                                echo "Test failed"
                            }
                        }
